<div>
                ...
                <div class="card-header p-4">
                    <div class="row align-items-center">
                        <div class="col">
                            ...
                            <p class="text-sm mb-0">{{$phone ? getCountryCode() : ''}} {{ $phone }}</p>
                        ...
                        <div class="col-auto mt-4">
                            ...
                            <p class="text-sm mb-3">
                                ...
                                <span
                                    ...
                                    <a data-bs-toggle="modal" data-bs-target="#changeDeliveryDate"
                                                     type="button"
                                                    class="badge badge-xs badge-secondary text-xs fw-600 customer-btn-hover">
                            ...
                            <tbody>
                                @foreach ($orderdetails as $item)
                                    ...
                                    <tr>
                                        ...
                                        <td class="">
                                            <p class="text-sm px-3 mb-0">{{ getFormattedCurrency($item->service_price) }}</p>
                                        </td>
                                        ...
                                        <td class="">
                                            <p class="text-sm px-3 mb-0">
                                                {{ getFormattedCurrency($item->service_detail_total) }}</p>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
                ...
                <div class="card-footer px-4">
                    <div class="row">
                        <div class="col-md-8 mb-3">
                            ...
                            <p class="text-sm mb-0">{{$customer && $customer->phone ? getCountryCode() : ''}} {{ $customer->phone ?? 'Phone' }}</p>
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <div class="">
                                <div class="row mb-50 align-items-center">
                                    ...
                                    <div class="col-auto text-sm">{{ getFormattedCurrency($order->sub_total) }}</div>
                                </div>
                                <div class="row mb-50 align-items-center">
                                    <div class="col text-sm">{{ $lang->data['addon'] ?? 'Addon' }}:</div>
                                    <div class="col-auto text-sm">{{ getFormattedCurrency($order->addon_total) }}</div>
                                </div>
                                <div class="row mb-50 align-items-center">
                                    <div class="col text-sm">{{ $lang->data['discount'] ?? 'Discount' }}:</div>
                                    <div class="col-auto text-sm">{{ getFormattedCurrency($order->discount) }}</div>
                                </div>
                                <div class="row mb-3 align-items-center">
                                    <div class="col text-sm">{{ $lang->data['tax'] ?? 'Tax' }}
                                        ({{ $order->tax_percentage }}%):</div>
                                    <div class="col-auto text-sm">{{ getFormattedCurrency($order->tax_amount) }}</div>
                                </div>
                                <div class="row align-items-center">
                                    <div class="col text-sm fw-600">
                                        {{ $lang->data['gross_total'] ?? 'Gross Total' }}:
                                    </div>
                                    <div class="col-auto text-sm text-dark fw-600">{{ getFormattedCurrency($order->total) }}</div>
                                </div>
                            </div>
                        </div>
                                    ...
                                    <div class="d-flex flex-column">
                                        @foreach ($orderaddons as $item)
                                            <span class="mb-3 text-sm">
                                                ...
                                                <span class="text-sm ms-2">{{ getFormattedCurrency($item->addon_price) }}</span>
                                            </span>
                                        @endforeach
                                    </div>
                                </li>
                        ...
                        @foreach ($payments as $item)
                            <div class="timeline-block mb-3">
                                ...
                                <div class="timeline-content">
                                    <h6 class="text-dark text-sm font-weight-bold mb-0">{{ getFormattedCurrency($item->received_amount) }}</h6>
    ...
    <div class="modal fade " id="addpayment" tabindex="-1" role="dialog" aria-labelledby="addpayment"
                        ...
                        <div class="row g-2 align-items-center">
                            <div class=" col-12">
                                ...
                                <div class="row mb-50 align-items-center">
                                    ...
                                    <div class="col-auto  text-sm fw-500">{{ getFormattedCurrency($order->total) }}</div>
                                </div>
                                <div class="row mb-50 align-items-center">
                                    <div class="col text-sm fw-500">
                                    ...
                                    <div class="col-auto text-sm fw-500">
                                        {{ getFormattedCurrency($order->total - $balance, 2) }}
                                        </div>
                                </div>
                                <hr>
                                <div class="row align-items-center">
                                    <div class="col text-sm fw-600">{{ $lang->data['balance'] ?? 'Balance' }}:</div>
                                    <div class="col-auto text-sm fw-600">{{ getFormattedCurrency($balance) }}</div>
                                </div>
                                <hr>
                                <div class="row align-items-center">
        ...
        <!-- delivery date change section   -->
        <div class="modal fade " id="changeDeliveryDate" tabindex="-1" role="dialog" aria-labelledby="changeDeliveryDate"
        aria-hidden="true" wire:ignore.self>
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h6 class="modal-title fw-600" id="addpayment">
                        {{ $lang->data['delivery_date'] ?? 'Delivery Date' }}</h6>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <form>
                        <div class="modal-body">
                            <div class="row g-2 align-items-center">
                                <div class=" col-12">
                                    <div class="col-12">
                                            <input type="date" class="form-control" wire:model="current_delivery_date">
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="submit" class="btn btn-primary"
                            wire:click.prevent="changeDeliveryDate()">{{ $lang->data['save'] ?? 'Save' }}</button>
                    </div>
                </form>
            </div>
        </div>
